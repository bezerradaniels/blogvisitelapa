...
export default function HomeSectionManager({ sections, initialPosts }: { sections: HomeSection[]; initialPosts: Record<string, SearchPost[]> }) {
  ...
  function createNew() { setForm(empty()); setError(null); setTerm(''); setResults([]); setEditorOpen(true); }
  function edit(section: HomeSection) { setForm(fromSection(section, initialPosts[section.id]?.map((post) => post.id) ?? [])); setError(null); setTerm(''); setResults([]); setEditorOpen(true); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  function closeEditor() { setEditorOpen(false); setForm(empty()); setTerm(''); setResults([]); setError(null); }
